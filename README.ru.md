...
- **Распознавание лиц** — детектор SCRFD-10GF + AdaFace IR-101 (512-мерные эмбеддинги float32), CLAHE-препроцессинг, выравнивание по 5 ключевым точкам
...
- **float32 эмбеддинги** — 2048 байт каждый, с автоматической конвертацией устаревших float64
...
1. Создаёт `.env` со сгенерированными `SECRET_KEY` и `INITIAL_INVITE_CODE`
...
| Размерность | 512 |
| Тип данных | float32 |
| Размер | 2048 байт на сотрудника |
| Хранение | SQLite LargeBinary |
| Обратная совместимость | Автоопределение float64 (4096 байт) с конвертацией |
...
|-------|----------|-------------|----------|
| POST | `/api/v1/employees/register` | Да | Регистрация с фото лица |
...
- **Безопасная десериализация** эмбеддингов с автоопределением float64 legacy
- **pytest** — 20+ тестов с in-memory SQLite
